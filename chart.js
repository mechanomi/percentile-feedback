...
(function() {
  var chart_data, chart_local_start_hour, check_cors_setup, cors_succeeded, draw_chart, floor_to_day, floor_to_hour, floor_to_minute, floor_to_seconds, format_seconds, histogram_to_efficiencies, isotime_to_unixtime, loaded, mean_percentile_rank, prepare_data, query_succeeded, root, sync_succeeded, timezone, unixtime_now, utc_midnight;
  ...
  draw_chart = function(data) {
    var height, line, margin, outerHeight, outerWidth, svg, width, xAxis, xMap, xMapJitter, xScale, yAxis, yMap, yScale;
    $("svg").remove();
    margin = {
      top: 10,
      right: 10,
    ...
    svg.append("g").attr("class", "history").selectAll("circle").data(data.merged).enter().append("circle").attr({
      cx: xMapJitter,
      cy: yMap,
      r: 4
    }).style({
    ...
    svg.append("defs").append("clipPath").attr("id", "clip").append("rect").attr("width", xScale(data.hours)).attr("height", height + 20).style("fill", "blue");
    svg.append("g").attr("class", "today line").datum(data.today.slice(0)).append("path").attr("clip-path", "url(#clip)").attr("d", line);
    return svg.append("text").attr("class", "pr").attr("x", 20).attr("y", 40).text("PR " + data.pr);
  };
  ...
  prepare_data = function(history, today) {
    var efficiency, hours, mean_pr, merged, now, value, values, view, _i, _j, _len, _len1, _ref;
    merged = [];
    ...
    return {
      ...
      today: today,
      hours: hours,
      pr: Math.floor(mean_pr)
    };
  };

  chart_data = function(views, data) {
    var sum, today_duration, total_duration, view;
    draw_chart(data);
  ...
  query_succeeded = function(doc) {
    var bins, create_view, current_view, data, histogram, minute, n, row, start_minute, unixtime, view, view_start, view_tick, view_width, views, _i, _j, _k, _l, _len, _len1, _ref, _ref1, _ref2, _ref3;
    views = [];
    ...
    return root.pf.action(views, data);
  };

  sync_succeeded = function(db) {
    return db.allDocs({
      include_docs: true,
      ascending: true
    }, function(err, doc) {
  ...
  cors_succeeded = function() {
    var database_name, db;
    database_name = "pf-periods";
    db = PouchDB(database_name);
    return db.replicate.from("http://127.0.0.1:5984/" + database_name, {
      live: true
    }).on("uptodate", function() {
      console.log("WAYPOINT: Replicated remote DB", unixtime_now() - loaded);
      return sync_succeeded(db);
    });
  };

  check_cors_setup = function(cors_succeeded, cors_failed) {
    return $.get("http://127.0.0.1:5984/_config/cors/credentials").done(function(arg) {
      var text;
      text = JSON.parse(arg);
      if (text !== "true") {
        return cors_failed();
      } else {
        return cors_succeeded();
      }
    }).fail(function(arg) {
      return cors_failed();
    });
  ...
  root.pf.compute = function() {
    var show_cors_error_message;
    show_cors_error_message = function() {
      return $("body").append("<p>You must set up CORS correctly!</p>");
    };
    return check_cors_setup(cors_succeeded, show_cors_error_message);
  };
